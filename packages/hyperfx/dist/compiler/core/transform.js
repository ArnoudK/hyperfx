...
export class HyperFXTransformer {
     ...
     */
    transformStaticElement(node, _path, s) {
     ...
     */
    transformDynamicElement(node, _path, s) {
     ...
     */
    generateElementAccess(rootVar, path) {
        if (path.length === 0) {
            return rootVar;
        }
        let code = rootVar;
     ...
     */
    generateDynamicCode(templateId, dynamics, _node) {
        const lines = [];
        // Clone the template
        lines.push(`(() => {`);
