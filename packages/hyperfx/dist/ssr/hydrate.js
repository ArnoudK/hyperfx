 ...
 */
export function hydrate(vnode, container, hydrationData) {
    if (!hydrationData) {
 ...
 */
function hydrateVNode(vnode, container, hydrationData) {
    const markers = hydrationData.markers;
    let markerIndex = 0;
    function findHydratedElement(node) {
        ...
        node.children.forEach(child => {
            if (typeof child === 'object' && 'tag' in child) {
                ...
                if (node.children.length === 1) { // If the signal is the only child of its VNode parent
                    createEffect(() => {
                        if (element)
                            element.textContent = String(signal());
                    });
                }
                else {
                    // Signal is one of multiple children of VNode `node`.
                    // `element` is the DOM corresponding to `node`.
                    console.warn(`[hydrateNode] A reactive text signal was found among multiple children of <${String(node.tag)}>. ` +
 ...
 */
export function autoHydrate(componentFunction, containerElement = document.body) {
    const isInitialSsrLoad = !(window.__hyperfx_client_navigated__) && containerElement.firstChild;
    ...
    if (isInitialSsrLoad) {
        ...
        if (containerElement.tagName === 'BODY') {
            ...
            const vnodeClasses = vnode.props?.class;
            if (vnodeClasses) {
                const mainDiv = containerElement.querySelector(`div.${vnodeClasses.split(' ')[0]}`);
                if (mainDiv) {
 ...
 */
function hydrateInteractiveComponents(vnode, container) {
    // Find all elements with data-hyperfx-hydrate markers (these have event handlers)
    const interactiveElements = Array.from(container.querySelectorAll('[data-hyperfx-hydrate]'));
    ...
    const interactiveVNodes = new Map();
    let vnodeIndex = 0;
    function collectInteractiveVNodes(node) {
        if (typeof node === 'string' || typeof node === 'function') {
            return;
        }
        if (typeof node === 'object' && node !== null && 'tag' in node && typeof node.tag === 'string') {
            const vNodeInstance = node;
            const props = vNodeInstance.props || {};
    ...
    interactiveElements.forEach(element => {
        ...
        if (vNodeToHydrate && vNodeToHydrate.props) {
            const props = vNodeToHydrate.props;
            ...
            Object.entries(props).forEach(([key, value]) => {
                ...
                if (key.startsWith('on') && typeof value === 'function') {
                    const eventName = key.slice(2).toLowerCase();
                    // Remove old listener if any
                    const oldHandler = element[`__event_${eventName}`];
                    if (oldHandler) {
                        element.removeEventListener(eventName, oldHandler);
                    }
                    // Add new listener
                    element.addEventListener(eventName, value);
                    element[`__event_${eventName}`] = value;
 ...
 */
function attachEventHandlers(vnode, containerElement) {
    // Create a map of vnodes by traversing the tree in the same order as the server
    const vnodeMap = new Map();
    let vnodeMapIndex = 0;
    function mapVNodes(node) {
        if (typeof node === 'string' || (typeof node === 'function' && !node.__isReactiveComponent)) {
            return;
        }
        if (typeof node === 'object' && node !== null && 'tag' in node && typeof node.tag === 'string') {
            ...
            if (hasEventHandlers) {
                vnodeMap.set(vnodeMapIndex, node);
            }
            vnodeMapIndex++;
            if (node.children && Array.isArray(node.children)) {
                node.children.forEach(child => mapVNodes(child));
            }
        }
    }
    mapVNodes(vnode);
    // Attach event handlers to DOM elements with static hydration markers
    const hydratedElements = Array.from(containerElement.querySelectorAll('[data-hyperfx-hydrate]'));
    hydratedElements.forEach((element) => {
        const hydrateIndexStr = element.getAttribute('data-hyperfx-hydrate');
        if (hydrateIndexStr === null)
            return;
        const hydrateIndex = parseInt(hydrateIndexStr, 10);
        ...
        if (vNodeToHydrate && vNodeToHydrate.props) {
            Object.entries(vNodeToHydrate.props).forEach(([key, value]) => {
                if (key.startsWith('on') && typeof value === 'function') {
                    ...
                    // Remove old listener if any
                    const oldHandler = element[internalEventKey];
                    if (oldHandler) {
                        element.removeEventListener(eventName, oldHandler);
                    }
                    // Add new listener
                    element.addEventListener(eventName, value);
                    element[internalEventKey] = value;
                }
            });
            element.removeAttribute('data-hyperfx-hydrate');
        }
 ...
 */
function linkVNodesToDOM(vnode, parentElement) {
    ...
    if (vnode.tag === FRAGMENT_TAG) {
        ...
        if (vnode.children) {
            let childIndex = 0;
            const elementChildren = Array.from(parentElement.children).filter(el => el instanceof HTMLElement);
 ...
 */
function setupReactiveEffects(node, parentElement) {
    if (typeof node === 'string' || (typeof node === 'function' && !node.__isReactiveComponent && !node.__isReactiveExpression)) {
        return;
    }
    if (typeof node === 'object' && node !== null && 'tag' in node && typeof node.tag === 'string') {
        const vNodeInstance = node;
        // Handle reactive For components
        if (vNodeInstance.props?.['data-reactive-for'] === 'true' && vNodeInstance.__reactiveArrayFn && vNodeInstance.__renderFn) {
            const reactiveId = vNodeInstance.props['data-reactive-id'];
            let forContainer = document.querySelector(`[data-reactive-id="${reactiveId}"]`);
            ...
            if (!forContainer) {
                forContainer = document.querySelector(`[data-reactive-for="true"]`);
                if (forContainer) {
            ...
            const reactiveArrayFn = vNodeInstance.__reactiveArrayFn;
            const renderFn = vNodeInstance.__renderFn;
            const fallback = vNodeInstance.__fallback;
            let previousKeyedElements = new Map();
            ...
            const initializeKeyedElements = () => {
                try {
                    ...
                    if (Array.isArray(currentArrayData)) {
                        const existingChildren = Array.from(forContainer.children);
                        currentArrayData.forEach((item, index) => {
                            let key;
            ...
            const effectCleanup = createEffect(() => {
                ...
                try {
                    ...
                    if (currentArrayData.length === 0 && fallback) {
                        ...
                        if (fallbackVNode && fallbackVNode.key === undefined)
                            fallbackVNode.key = '__fallback__';
                        if (fallbackVNode)
                            newVNodes.push(fallbackVNode);
                    }
                    else {
                        currentArrayData.forEach((item, index) => {
                    ...
                    Array.from(forContainer.children).forEach(childElement => {
                        const childHtmlElement = childElement;
        ...
        if (vNodeInstance.props?.['data-reactive-expr'] === 'true' && vNodeInstance.__reactiveExprFn) {
            const reactiveId = vNodeInstance.props['data-reactive-id'];
            const exprContainer = document.querySelector(`[data-reactive-id="${reactiveId}"]`);
            const reactiveExprFn = vNodeInstance.__reactiveExprFn;
            if (exprContainer) {
                createEffect(() => {
                    try {
                        const content = reactiveExprFn();
                        const contentArray = Array.isArray(content) ? content : [String(content)];
                        reconcileChildren(exprContainer, contentArray);
                    }
                    catch (error) {
        ...
        if (vNodeInstance.props?.['data-reactive-text'] === 'true' && vNodeInstance.__reactiveTextFn) {
            const reactiveId = vNodeInstance.props['data-reactive-id'];
            console.log('🔍 Setting up reactive text marker with ID:', reactiveId);
            let textContainer = document.querySelector(`[data-reactive-id="${reactiveId}"]`);
            ...
            if (!textContainer) {
                ...
                textContainer = document.querySelector(`[data-reactive-text="true"]`);
                if (textContainer) {
            ...
            if (textContainer) {
                ...
                const reactiveTextFn = vNodeInstance.__reactiveTextFn;
        ...
        if (vNodeInstance.props?.['data-reactive-conditional'] === 'true' && vNodeInstance.__reactiveConditionalFn) {
            const reactiveId = vNodeInstance.props['data-reactive-id'];
            console.log('🔍 Setting up reactive conditional with ID:', reactiveId);
            let conditionalContainer = document.querySelector(`[data-reactive-id="${reactiveId}"]`);
            if (conditionalContainer) {
                console.log('✅ Found reactive conditional container:', conditionalContainer);
                const reactiveConditionalFn = vNodeInstance.__reactiveConditionalFn;
        ...
        if (vNodeInstance.children && Array.isArray(vNodeInstance.children)) {
            const currentElement = vNodeInstance.dom;
            vNodeInstance.children.forEach((child, childIndex) => {
                if (typeof child === 'object' && child && 'tag' in child) {
 ...
 */
function setupMixedTextReactivity(element, children, signalIndex, signal) {
...
function updateDomElementProperties(element, props) {
    const elProps = props || {};
    ...
    currentAttributes.forEach(attrName => {
        if (attrName !== 'data-key' && !attrName.startsWith('on') && attrName !== 'style' && !(attrName in elProps)) {
            if (attrName === 'class' && ('className' in elProps))
                return;
            if (attrName === 'for' && ('htmlFor' in elProps))
                return;
            element.removeAttribute(attrName);
        }
    });
    Object.entries(elProps).forEach(([key, value]) => {
        if (key === 'children' || key === 'key' || key.startsWith('__') || key === 'ref' || key === 'innerHTML')
            return;
        if (key.startsWith('on') && typeof value === 'function') {
            const eventName = key.slice(2).toLowerCase();
            const internalEventKey = `__event_${eventName}`;
...
function setElementProperty(element, key, value) {
    if (key === 'style' && typeof value === 'object' && value !== null) {
        ...
        if (element.className !== String(value))
        ...
        if (key === 'value') {
            ...
            if (targetElement.checked !== value)
                targetElement.checked = value;
        }
        else if (key === 'selected' && 'selected' in targetElement) {
            if (targetElement.selected !== value)
                targetElement.selected = value;
        }
...
function reconcileChildren(parentElement, newChildrenVNodes) {
...
function updateElementInPlace(element, vnode) {
    updateDomElementProperties(element, vnode.props);
    reconcileChildren(element, vnode.children);
