...
export class HyperFXTransformer {
    constructor(options = {}) {
        ...
        this.templatesByHTML = new Map(); // HTML -> templateId for deduplication
     ...
     */
    generateElementCodeInline(templateId, dynamics, node) {
        const lines = [];
        // Clone the template
        lines.push(`const _el$ = ${templateId}.cloneNode(true);`);
        // Handle dynamic attributes first
    ...
    tryOptimizeMapCall(node) {
        ...
        if (t.isArrowFunctionExpression(mapFn) || t.isFunctionExpression(mapFn)) {
            ...
            const params = mapFn.params.map((p) => this.codeFromNode(p)).join(', ');
            let body;
    ...
    codeFromNode(node, context) {
        ...
        try {
            ...
            if (t.isCallExpression(node)) {
                ...
                const args = node.arguments.map((arg) => this.codeFromNode(arg)).join(', ');
                return `${callee}(${args})`;
            }
